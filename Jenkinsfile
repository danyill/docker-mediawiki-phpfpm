    ...
    try {
      ...
      docker.image("${maintainer_name}/${container_name}:${build_tag}").withRun("--name=${container_name} -d -p 127.0.0.1:8000:8000", "php -S 0.0.0.0:8000 -t / /phpinfo.php" )  { c ->
         ...
         for (test_num = 0; test_num < MAX_TESTS; test_num++) {
             ...
             if (test_num == 0 )
             {
                 test_results = sh([script: "curl -s 127.0.0.1:8000 | grep -o 'PHP Version 7.1'", returnStatus:true])
                 build_tag = sh([script: $/curl -s 127.0.0.1:8000 | grep -o "PHP Version 7.1.[0-9]*" | grep -o 7.1.[0-9]*/$, returnStdout: true])
                 expected_results = 0
             }
             else if (test_num == 1)
             {
